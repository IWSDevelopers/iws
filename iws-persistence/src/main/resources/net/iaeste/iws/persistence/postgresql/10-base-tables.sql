...
--   If the expires date is set, then the alias will expire at this point in
-- time. If not set, then the alias will not expire - this means that any alias
-- without an expiration date is a permanent alias.
--   Together with the alias, the status is having a unique constraint, meaning
-- that the same alias can only be active once. This will allow that an alias is
-- created again later. Once the alias expires, then the status must be changed
-- to a UUID. Which will then be interpreted as the alias is no longer active.
...
-- in question.
--   Note, an alias for a private group is a personal alias and not a group
-- alias.
-- =============================================================================
...
-- Mailing lists
-- -----------------------------------------------------------------------------
-- The Mailing lists table holds information about both user aliases and various
-- mailing lists. Since mailing lists can be generated from many sources, it is
-- best to try to keep things simple.
--   The address is fixed with the rule, that it can be either private or public
-- depending on the domain used. the .net domain is for private lists and the
-- .org domain is for public lists. Aliases is also filled into this table via
-- various schedulers.
--   To minimize queries, user aliases, which is a public address with only a
-- single recipient, is also present in this table. To better distinguish
-- between lists, there is also a list type, which can be either LIST, ALIAS
-- or PRIVATE. If it is an ALIAS, then the rule applies that it may only live
-- for a brief period of time defined by the alias setup in the aliases table.
-- PRIVATE lists are simply the old style "user alias", and LIST is the
-- remainder.
--   The ReplyTo Style, is used to determine of the replies should go to the
-- list or to the sender. By default, it should always go to the sender, unless
-- it is a private mailing list, where the ReplyTo setting should be the list.
-- However, it is also possible that no replies should be made. E-mails to the
-- Announcement list should not have a ReplyTo setting.
-- =============================================================================
...
-- =============================================================================
-- User to Mailing List
-- -----------------------------------------------------------------------------
-- The list of members on a mailing list. Although it is possible to read out
