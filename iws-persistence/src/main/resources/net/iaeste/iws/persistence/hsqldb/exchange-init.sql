...
CREATE SEQUENCE study_levels_sequence START WITH 1 INCREMENT BY 1;
CREATE TABLE study_levels (
    id                     INT GENERATED BY DEFAULT AS SEQUENCE study_levels_sequence PRIMARY KEY,
    name                   VARCHAR(100) NOT NULL,
    offer_id               INTEGER NOT NULL REFERENCES offers (id) ON DELETE CASCADE,
    modified               TIMESTAMP    DEFAULT now(),
    created                TIMESTAMP    DEFAULT now()
...
CREATE SEQUENCE specializations_sequence START WITH 1 INCREMENT BY 1;
CREATE TABLE specializations (
    id                     INT GENERATED BY DEFAULT AS SEQUENCE specializations_sequence PRIMARY KEY,
    name                   VARCHAR(100) NOT NULL,
    offer_id               INTEGER NOT NULL REFERENCES offers (id) ON DELETE CASCADE,
    modified               TIMESTAMP    DEFAULT now(),
    created                TIMESTAMP    DEFAULT now()
