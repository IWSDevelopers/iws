...
public final class GenerateDataForTest {
    ...
    private static final String SHARE_ALL_OFFER = "insert into offer_to_group (offer_id, group_id, external_id) select offers.id, groups.id, offers.id || 'what_ever' || groups.id from groups, offers left join employers on offers.employer_id = employers.id where groups.grouptype_id = %d and employers.group_id != groups.id;";
