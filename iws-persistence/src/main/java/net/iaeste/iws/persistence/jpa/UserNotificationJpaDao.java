 ...
 * @since 1.7
 */
public class UserNotificationJpaDao extends BasicJpaDao implements UserNotificationDao {
    /**
     * Default Constructor.
     *
     * @param entityManager  Entity Manager instance to use
     */
    public UserNotificationJpaDao(final EntityManager entityManager) {
        super(entityManager);
    }
     ...
     */
    @Override
    public UserNotificationEntity findUserNotificationSetting(User user, NotificationSubject subject) {
        ...
        if (result.size() != 1) {
            throw new IWSException(IWSErrors.AUTHORIZATION_ERROR, "No user notification (" + subject + ") for the user '" + user.getUsername() + "' was found.");
        }
     ...
     */
    @Override
    public List<NotificationMessageEntity> findNotificationMessages(NotificationType type, NotificationMessageStatus status, Date date) {
     ...
     */
    @Override
    public void updateNotificationMessageStatus(NotificationMessageEntity message, NotificationMessageStatus status) {
